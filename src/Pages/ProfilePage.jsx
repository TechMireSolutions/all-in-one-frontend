    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-300 py-4 flex items-center justify-center">
      ...
      >
        ...
        <div className="flex-1 space-y-6">
          ...
          {/* Education */}
          {(shouldDisplay(degree) || shouldDisplay(institute) || shouldDisplay(grade) || shouldDisplay(year) || shouldDisplay(current_study)) && (
            <Section title="Education">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                {shouldDisplay(current_study) && <ProfileItem icon={<FaBookOpen />} label="Current Study (Ongoing)" value={current_study} />}
              </div>
            </Section>
          )}
...
const Section = ({ title, children }) => (
  <div className="bg-gray-50 p-3.5 rounded-xl shadow-inner border border-gray-100">
    <h3 className="text-lg font-semibold text-gray-700 mb-2.5">{title}</h3>
    {children}
  </div>
);
...
const ProfileItem = ({ icon, label, value }) => (
  <motion.div
    ...
    className="flex items-start space-x-3 bg-white p-2.5 rounded-lg shadow-sm border border-gray-200 min-w-0"
  >
    <span className="text-orange-500 mt-1 shrink-0">{icon}</span>
    <div className="min-w-0 flex-1">
      <p className="text-xs font-medium text-gray-500 mb-0.5">{label}</p>
      <p className="text-sm text-gray-800 font-semibold break-words whitespace-normal leading-tight" title={value}>
        {value}
      </p>
    </div>
  </motion.div>
