  ...
  if (role?.toLowerCase() !== "superadmin" && Array.isArray(user?.allowedPages)) {
    ...
    const hasProjectTrackerAccess =
      ...
      (user.allowedPages.includes(PROJECT_TRACKER_EDIT_KEY) ||
        user.allowedPages.includes(PROJECT_TRACKER_VIEW_KEY));
    if (!hasProjectTrackerAccess && !user.allowedPages.includes(path)) {
        ...
        <Routes>
          ...
          {user && (
            <>
              ...
              {role?.toLowerCase() === "hr" && (
                <>
                  <Route path="/project-tracker" element={<ProtectedRoute allowedRoles={["hr"]}><ProjectProgressTracker /></ProtectedRoute>} />
